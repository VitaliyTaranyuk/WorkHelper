  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        ...
        Есть режим рации (push-to-talk): удерживайте {formatHotkey(hotkey)}
        — запись идёт, пока клавиши зажаты, отпускание сразу завершает запись и
        запускает обработку. Говорите свободно — весь текст накапливается до конца
        записи, паузы не обрывают диктовку. Чтобы завершить обычную запись,
        скажите «Работаем» (стоп-фраза) или нажмите «Стоп». Изменения данных и
        опасные операции выполняются после подтверждения; последнее действие
        можно отменить в журнале команд.
      </Typography>
