 ...
 *
 * Счётчики собираются в памяти и периодически выводятся в лог сводкой: этого
 * достаточно, чтобы ответить на вопросы «часто ли деградируем» и «сколько это
 * стоит».
 *
 * T-301 подключил Actuator и Micrometer, но эти счётчики на них НЕ переведены —
 * задача добавляла процессные метрики (JVM, пул соединений, http.server.requests),
 * а переписывать работающую доменную подсистему в неё не входило (K-23).
 * Записано как TD-048: сейчас в проекте два места, где живут метрики.
 *
