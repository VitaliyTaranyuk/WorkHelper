...
| TD-021 | Деплой бэкенда не автоматизирован и не описан: README обещает `.github/workflows/backend-deploy.yml`, `frontend-deploy.yml` и K3s-кластер, но в репозитории только `backend-ci.yml`/`frontend-ci.yml`, а деплой-скриптов не было ни разу за всю историю (`git log --all --diff-filter=A -- "*deploy*"`) | Документация опережала реализацию | HIGH: смерженный код может неограниченно долго не доходить до пользователя, и это НЕ видно из репозитория. Обнаружено при аудите ТП-212: ТП-208 (`POST /voice/enhance-text`) смержен в `main` 2026-07-26, но на `wowoffcata.hlab.kz` эндпоинт отвечает 401 так же, как несуществующий путь, — то есть на VDS работает образ, собранный до ТП-208. Фикс: workflow ручного деплоя по SSH (секреты `SERVER_IP`/`SERVER_USER`/`SSH_PRIVATE_KEY` уже описаны в README) либо честное описание ручной процедуры `docker compose -f docker-compose.vds.yml up -d --build backend` | OPEN |
| TD-023 | `backend/Dockerfile` копирует готовый `app.jar` (`COPY app.jar app.jar`), а не собирает исходники; `app.jar` в git не хранится | Историческая схема «собери где-нибудь и положи рядом» | HIGH: `git pull` + `docker compose up --build` на сервере НЕ обновляют приложение — образ переупаковывает старый jar, и это выглядит как успешный деплой. Именно так ТП-208 «деплоилась» три недели (проверено 2026-07-27: после pull и пересборки эндпоинт оставался 401, помогла только ручная сборка jar и заливка). Java на VDS нет, свободной памяти ~690 МБ — собирать на сервере нельзя. Фикс: multi-stage Dockerfile (сборка внутри образа) либо шаг сборки+заливки jar в пайплайне TD-021 | OPEN |
