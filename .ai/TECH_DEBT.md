...
| TD-019 | JWT-секрет (`spring.app.jwtSecret`) захардкожен в `application.yml` (`mySecretKey123...`), общий для всех профилей включая prod — виден любому с доступом к репозиторию | Историческое; секрет никогда не выносился в окружение | HIGH: обладая ключом, можно подделать JWT и авторизоваться под любым пользователем в проде | OPEN — найдено при работе над ТП-210, вынесено отдельной задачей по решению пользователя (2026-07-26); фикс требует выноса в обязательную env-переменную + смены секрета на реальном сервере при деплое (разлогинит текущие сессии) |
