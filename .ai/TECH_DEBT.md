...
| TD-017 | Голос: командный режим полноценно работает только с LLM (I2/ТП‑96); без ключа доступен лишь ограниченный набор через RuleBased | Ключ AI (ТП‑10) — внешняя зависимость; сознательно отложен | MEDIUM: архитектура допускает подключение LLM позже без переработки (шов `IntentResolver`, авто‑фолбэк); не блокирует Веху 1 | OPEN |
